...
<body>
	...
	$upit = "select * from artikal where SoljaMajica = 1";
	$rez = $mysqli->query($upit);

	if(!$rez){
		print("Trenutno nema šolja!");
		die($mysqli->error);
	}
	...
	if(!$red){
		...
		if((isset($_SESSION['rola']) && $_SESSION['rola'] == 0) || (isset($_SESSION['rola']) && $_SESSION['rola'] == 1)){
			$message .= "
			<div class='majice'>
			<a href='artikal.php?artikalId=" . $sifraArtikla . "'><img src=\"" . $fajl . "\" alt='šolja" . $sifraArtikla . "'></a>
			<h1>" . $boja . " šolja</h1>
			<p>Cena " . $cena . "</p>
			</div>
			";
			...
			$message .= "
			<div class='majice'>
			<a href='login.php'><img src=\"" . $fajl . "\" alt='šolja" . $sifraArtikla . "'></a>
			<h1>" . $boja . " šolja</h1>
			<p>Cena " . $cena . "</p>
			</div>
			";
